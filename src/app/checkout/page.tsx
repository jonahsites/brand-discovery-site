...
function AddrField({ k, addr, setAddr, span, placeholder }: { k: keyof Addr; addr: Addr; setAddr: (fn: (a: Addr) => Addr) => void; span?: boolean; placeholder?: string }) {
  return <input value={addr[k]} onChange={(e) => setAddr((a) => ({ ...a, [k]: e.target.value }))} placeholder={placeholder} className={clsx(inputCls, "!bg-offwhite !border-transparent focus:!border-black/30", span && "sm:col-span-2")} />;
}
  ...
  if (done) return (
    <Page narrow className="pt-16 text-center">
      <div className="mx-auto max-w-[560px] rounded-lg bg-sky p-10">
        <div className="label mb-4 !text-black/48">Order #{done.id} placed</div>
        <h1 className="mb-3 text-[34px] font-bold leading-[1.05] tracking-[-.04em]">Thanks, {addr.name.split(" ")[0]}.</h1>
        <p className="mb-6 text-[14.5px] leading-[1.55] text-black/66">{new Set(done.items.map((i) => i.brand)).size} parcels from {new Set(done.items.map((i) => i.brand)).size} workshops, {money(done.total, true)} total. Kindred holds payment until each brand scans your parcel.</p>
    ...
    <Page className="pt-6 md:pt-[34px]">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_400px] items-start">
          ...
          <div className="card mb-4 rounded-lg p-5 md:p-[30px]">
            ...
            {bagGroups.map((g) => { const s = SHIP_OPTS[g.brand.slug] ?? { from: g.brand.shipsFrom, opts: [{ label: "Standard", meta: "5–8 days · $9.00", cost: 9 }] }; const cur = ship[g.brand.slug] ?? 0; return (
              <div key={g.brand.slug} className="border-t border-black/7 py-[18px]">
                ...
                <div className="flex gap-[10px]">{s.opts.map((o, oi) => <button key={o.label} onClick={() => setShip(g.brand.slug, oi)} className={clsx("flex-1 rounded-md border-[1.5px] px-[18px] py-[15px] text-left", cur === oi ? "bg-offwhite border-black" : "bg-white border-black/9")}><div className="mb-1 text-[13px] font-semibold">{o.label}</div><div className="mono text-[11.5px] text-black/50">{o.meta}</div></button>)}</div>
              </div>); })}
          </div>
          <div className="card rounded-lg p-5 md:p-[30px]">
            <div className="mb-[22px]"><Step n={3} t="Payment" /></div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="relative sm:col-span-2"><input value={card.number} onChange={(e) => setCard((c) => ({ ...c, number: e.target.value }))} placeholder="Card number" className={clsx(inputCls, "!bg-offwhite !border-transparent focus:!border-black/30 pr-16")} /><span className="mono absolute right-4 top-1/2 -translate-y-1/2 text-[11px] text-black/40">VISA</span></div>
              <input value={card.exp} onChange={(e) => setCard((c) => ({ ...c, exp: e.target.value }))} placeholder="MM / YY" className={clsx(inputCls, "!bg-offwhite !border-transparent focus:!border-black/30")} />
              <input value={card.cvc} onChange={(e) => setCard((c) => ({ ...c, cvc: e.target.value }))} placeholder="CVC" className={clsx(inputCls, "!bg-offwhite !border-transparent focus:!border-black/30")} />
            </div>
            ...
            <div className="mt-3 text-[11.5px] text-black/40">Demo checkout. No card is charged; Stripe Connect lands with the backend.</div>
          </div>
        </div>
        <div className="glass rounded-lg p-6 md:p-7 lg:sticky lg:top-[100px]" style={{ backdropFilter: "blur(30px)" }}>
          ...
          {points >= 100 && <div className="mt-4 rounded-md bg-sky/60 p-3"><div className="mb-2 flex justify-between text-[12.5px]"><span className="font-semibold">Spend points</span><span className="text-black/55">{points.toLocaleString()} available · 100 = $1</span></div><input type="range" min={0} max={Math.min(points, Math.floor((subtotal + shipTotal - promoDiscount) * 100))} step={100} value={redeem} onChange={(e) => setRedeem(Number(e.target.value))} className="w-full accent-black" /><div className="mono mt-1 text-[11px] text-black/55">Using {redeem.toLocaleString()} points = {money(redeem / 100, true)}</div></div>}
