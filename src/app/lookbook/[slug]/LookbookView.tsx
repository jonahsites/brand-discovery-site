  ...
  return (
    <Page className="pt-6 md:pt-10">
      <div className="mb-6 md:mb-[34px] flex flex-col md:flex-row md:items-end justify-between gap-5">
        ...
        <div className="flex flex-none gap-[10px]"><Button size="lg" onClick={shopAll}>Shop all {shoppable}</Button><Link href={`/brand/${b.slug}`} className="grid h-12 w-12 place-items-center rounded-pill border border-black/10 bg-white text-[16px]">↗</Link></div>
      </div>
        ...
        {l.frames.map((f, i) => {
          ...
          return (
            <div key={i} className={clsx("relative overflow-hidden rounded-2xl md:rounded-lg", !f.bg && !f.image && "stripes-wide")} style={{ height: f.h * (i % 2 ? 0.8 : 0.85), background: f.bg }}>
              {f.image ? <img src={f.image} alt={f.label ?? `Look ${i + 1}`} className="absolute inset-0 h-full w-full object-cover" /> : <div className={clsx("mono absolute inset-0 grid place-items-center text-[10px] font-medium uppercase tracking-[.12em]", dark ? "text-offwhite/50" : "text-black/32")}>{f.label ?? `Look ${String(i + 1).padStart(2, "0")}`}</div>}
              ...
              {p && spot === i && (
                <div className="glass absolute inset-x-4 bottom-4 md:inset-x-5 md:bottom-5 flex items-center gap-[14px] rounded-xl p-[14px]">
                  ...
                  <button onClick={() => { addToBag(p.slug, `${(p.sizes ?? ["M"])[0]} · ${(p.colors ?? ["Default"])[0]}`); openBag(); }} className="press grid h-10 w-10 flex-none place-items-center rounded-pill bg-black text-[15px] text-white">+</button>
                </div>
              )}
            </div>
          );
      ...
      <div className="glass fixed inset-x-4 bottom-[100px] z-30 flex items-center gap-3 rounded-pill p-3 md:hidden"><div className="px-[10px] text-[12.5px] font-medium text-black/60">{shoppable} shoppable</div><Button className="flex-1" onClick={shopAll}>Shop the looks</Button></div>
      <div className="mt-10 flex items-center gap-3 rounded-lg bg-white p-4 border border-black/5"><Avatar init={b.init} tint={b.tint} ink={b.ink} size={40} src={b.logo} /><div className="flex-1 text-[13.5px]"><span className="font-semibold">{b.name}</span> <span className="text-black/50">· {b.city}, {b.country} · {looks} looks</span></div><Link href={`/brand/${b.slug}`} className="text-[12.5px] font-semibold text-navy">Visit brand →</Link></div>
    </Page>
  );
}
