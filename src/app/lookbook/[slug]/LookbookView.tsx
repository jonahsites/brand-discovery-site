            ...
            <div key={i} className={clsx("relative overflow-hidden rounded-lg", !f.bg && !f.image && "stripes-wide")} style={{ height: f.h * (i % 2 ? 0.8 : 0.85), background: f.bg }}>
              ...
              {p && spot === i && (
                <div className="card absolute inset-x-4 bottom-4 md:inset-x-5 md:bottom-5 flex items-center gap-[14px] rounded-md p-[14px]">
                  <Placeholder src={p.image} className="h-[66px] w-[58px] flex-none rounded-sm" />
                  ...
                  <button onClick={() => { addToBag(p.slug, `${(p.sizes ?? ["M"])[0]} · ${(p.colors ?? ["Default"])[0]}`); openBag(); }} className="press grid h-10 w-10 flex-none place-items-center rounded-pill bg-ink text-[15px] text-paper">+</button>
                </div>
              )}
            </div>
          );
