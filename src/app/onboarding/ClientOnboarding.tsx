...
import { STEP_COPY, STYLE_CHOICES, brandMeta } from "@/lib/data";
import { useApp } from "@/lib/store";
import { Avatar, Label } from "@/components/ui";
import { Fob, FobRow } from "@/components/Fob";

/** The Follow-brands step only shows when there are brands to follow. It gets pruned in
 *  `visibleSteps` below so onboarding doesn't dead-end on an empty picker. */
const ALL_STEPS = ["Style", "Sizes", "Brands"] as const;
...
export default function Onboarding() {
  ...
  const { brands: liveBrands, follows, toggleFollow, styleTags, setStyleTags, sizes, setSizes, completeOnboarding } = useApp();
  // Hide the follow step entirely when there's nothing to follow — the current state of a fresh
  // Kindred marketplace. It comes back the moment brands exist.
  const visibleSteps = liveBrands.length ? ALL_STEPS : ALL_STEPS.slice(0, 2);
  ...
  const next = () => { if (step === 0 && styles.length < 3) return; if (step === lastStep) void finish(); else setStep(step + 1); };
      ...
      <div className="grid place-items-center px-4 pb-20 md:pb-16">
        <div className="card flex w-full max-w-[520px] flex-col rounded-md p-6 md:p-8">
          <div className="mb-6 flex items-center gap-2">
            {visibleSteps.map((s, i) => <button key={s} onClick={() => i <= step && setStep(i)} className="flex-1 text-left" type="button"><div className={clsx("mb-[8px] h-[3px] rounded-sm", i <= step ? "bg-ink" : "bg-ink/12")} /><div className={clsx("text-[9.5px] font-semibold uppercase tracking-[.14em]", i <= step ? "text-ink" : "text-ink/38")}>{s}</div></button>)}
          </div>
          ...
          <div className="mb-6 flex-1">
            ...
            {step === 2 && <div className="flex flex-col gap-2">{liveBrands.map((b) => { const on = follows.includes(b.slug); return (
          ...
          <button type="button" onClick={next} disabled={step === 0 && styles.length < 3} className="press w-full rounded-sm bg-ink py-[11px] text-[12.5px] font-semibold text-paper disabled:opacity-40">{step === lastStep ? "Start browsing" : "Continue"}</button>
