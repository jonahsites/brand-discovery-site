      ...
      <div className="relative grid min-h-screen place-items-center p-4 md:p-[60px]">
        <div className="glass flex w-full max-w-[820px] flex-col rounded-3xl md:rounded-lg p-6 md:p-11" style={{ backdropFilter: "blur(34px)" }}>
          ...
          <div className="mb-8 md:mb-9 flex-1">
            {step === 0 && <div className="flex flex-wrap gap-2 md:gap-[10px]">{STYLE_CHOICES.map((c) => { const on = styles.includes(c); return <button key={c} onClick={() => setStyles(on ? styles.filter((x) => x !== c) : [...styles, c])} className={clsx("press rounded-pill border px-[19px] md:px-[26px] py-3 md:py-[15px] text-[13px] md:text-[14.5px] font-medium", on ? "bg-black text-white border-black" : "bg-white/80 border-black/10")}>{c}</button>; })}</div>}
