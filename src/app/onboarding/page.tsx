      ...
      <div className="relative grid min-h-screen place-items-center p-4 md:p-[60px]">
        <div className="glass flex w-full max-w-[820px] flex-col rounded-3xl md:rounded-lg p-6 md:p-11" style={{ backdropFilter: "blur(34px)" }}>
          <div className="mb-7 md:mb-8 flex items-center gap-2 md:gap-[10px]">
            {STEPS.map((s, i) => <button key={s} onClick={() => setStep(i)} className="flex-1 text-left"><div className={clsx("mb-[10px] h-1 rounded-pill", i <= step ? "bg-black" : "bg-black/13")} /><div className={clsx("hidden md:block text-[11px] font-semibold uppercase tracking-[.12em]", i <= step ? "text-ink" : "text-black/38")}>{s}</div></button>)}
          </div>
          <h1 className="mb-[10px] md:mb-3 text-[27px] md:text-[40px] font-bold leading-[1.1] md:leading-[1.05] tracking-[-.042em]">{title}</h1>
          <p className="mb-6 md:mb-8 max-w-[520px] text-[13.5px] md:text-[15.5px] leading-[1.6] text-black/60">{body}</p>
          <div className="mb-8 md:mb-9 flex-1">
            {step === 0 && <div className="flex flex-wrap gap-2 md:gap-[10px]">{STYLE_CHOICES.map((c) => { const on = styles.includes(c); return <button key={c} onClick={() => setStyles((prev) => prev.includes(c) ? prev.filter((x) => x !== c) : [...prev, c])} className={clsx("press rounded-pill border px-[19px] md:px-[26px] py-3 md:py-[15px] text-[13px] md:text-[14.5px] font-medium", on ? "bg-black text-white border-black" : "bg-white/80 border-black/10")}>{c}</button>; })}</div>}
            ...
            {step === 2 && <div className="grid gap-[10px] md:gap-[14px] md:grid-cols-2 lg:grid-cols-3">{BRANDS.map((b) => { const on = follows.includes(b.slug); return (
              <button key={b.slug} onClick={() => toggleFollow(b.slug)} className={clsx("flex items-center gap-[13px] rounded-md md:rounded-xl border-[1.5px] p-[13px] md:p-4 text-left", on ? "bg-white/92 border-black" : "bg-white/60 border-black/8")}>
                <Avatar init={b.init} tint={b.tint} ink={b.ink} size={46} />
                <div className="min-w-0 flex-1"><div className="mb-[3px] text-[13.5px] font-semibold">{b.name}</div><div className="mono text-[10.5px] text-black/45">{b.city} · {brandMeta(b).split(" · ")[0]}</div></div>
                <span className={clsx("grid h-[26px] w-[26px] flex-none place-items-center rounded-pill text-[12px] font-semibold text-white", on ? "bg-black" : "bg-black/12")}>{on ? "✓" : ""}</span>
              </button>); })}</div>}
          </div>
          <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
            <Button size="lg" className="md:!px-10" onClick={next}>{step === 2 ? "Start browsing" : "Continue"}</Button>
            <span className="text-center md:text-left text-[12px] md:text-[13px] text-black/45">{hint}</span>
            <button onClick={() => router.push("/")} className="md:ml-auto text-[13px] font-semibold text-black/40">Skip for now</button>
          </div>
        </div>
      </div>
    </div>
