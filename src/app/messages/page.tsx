          ...
          {thread && b ? (
            <>
              ...
              <div className="flex flex-1 flex-col gap-3 overflow-auto px-5 py-5">
                {thread.messages.map((m) => { const me = (m.from === "brand") === asBrand; return <div key={m.id} className={clsx("max-w-[78%] rounded-[14px] px-4 py-[10px] text-[13.5px] leading-[1.5]", me ? "self-end bg-ink text-white" : "self-start bg-offwhite")}>{m.text}<div className={clsx("mono mt-1 text-[9.5px]", me ? "text-white/50" : "text-ink/40")}>{new Date(m.at).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}</div></div>; })}
              </div>
