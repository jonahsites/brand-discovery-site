...
const NAV = ["Overview", "Products", "Promos", "Drops", "Orders", "Settings"];
...
function DashInner() {
  ...
  const app = useApp();
  const { session, brands, products, orders, promos, drops, hydrated, setSession } = app;
  ...
  return (
    <>
      ...
      <div className="mb-5 grid gap-5 xl:grid-cols-[1fr_380px] items-start">
        ...
        <div className="card rounded-lg p-6"><Label className="mb-4">New post</Label><textarea placeholder="Say something about this drop…" className="mb-3 min-h-[84px] w-full resize-none rounded-md bg-offwhite p-4 text-[13.5px] leading-[1.6] outline-none placeholder:text-black/42" /><div className="mb-4 flex gap-[9px]"><Placeholder className="h-[74px] flex-1 rounded-[9px]" /><div className="grid h-[74px] w-[74px] flex-none place-items-center rounded-[9px] border-[1.5px] border-dashed border-black/16 bg-offwhite text-[22px] font-light text-black/35">+</div></div><div className="flex gap-[9px]"><Button className="flex-1">Publish</Button><Button variant="secondary">Schedule</Button></div></div>
      </div>
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_420px] items-start">
      <div className="card rounded-lg p-4 md:p-[26px]">
        <div className="mb-4 flex items-center justify-between"><div className="text-[16px] font-semibold tracking-[-.02em]">Catalogue</div><Button size="sm" onClick={() => start()}>+ New product</Button></div>
...
function Settings({ brand }: { brand: string }) {
  ...
  const [f, setF] = useState({ tagline: b.tagline, story: b.story ?? "", moods: b.moods.join(", "), styles: b.styles.join(", "), materials: b.materials.join(", "), values: b.values.join(", "), website: b.website ?? "" });
  ...
  return (
    <div className="card max-w-[760px] rounded-lg p-5 md:p-[26px]">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3"><Button onClick={() => { upsertBrand({ ...b, tagline: f.tagline.trim(), story: f.story.trim(), website: f.website.trim() || undefined, styles: split(f.styles), moods: split(f.moods), materials: split(f.materials), values: split(f.values) }); setSaved(true); setTimeout(() => setSaved(false), 1500); }}>Save changes</Button>{saved && <span className="text-[12.5px] font-medium text-navy">Saved ✓</span>}</div>
      </div>
    </div>
  );
}
