...
function DashInner() {
  ...
  if (!brand) return <div className="grid min-h-screen place-items-center p-6 text-center"><div><h1 className="mb-2 text-[28px] font-extrabold tracking-[-.03em]">No brand account yet.</h1><Link href="/sell" className="font-semibold text-navy">Open one in five minutes →</Link></div></div>;
  ...
  const stats = [
    ...
    { label: "Followers", value: (brand.followers + (app.follows.includes(brand.slug) && !seeded ? 1 : 0)).toLocaleString(), delta: seeded ? "↑ 214 new" : "share your page", bg: "#DCD5C7", ink: "#121A24" },
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="hidden md:flex w-[252px] flex-none flex-col bg-navy p-5 pt-7 sticky top-0 h-screen">
        ...
        <div className="flex flex-col gap-[3px]">{NAV.map((l) => <button key={l} onClick={() => setNav(l)} className={clsx("rounded-sm px-[14px] py-[11px] text-left text-[13px] font-medium", nav === l ? "bg-offwhite/14 text-paper" : "text-paper/55")}>{l}</button>)}</div>
        <div className="mt-auto rounded-md border border-offwhite/14 bg-offwhite/10 p-[18px]"><div className="mb-[6px] text-[12.5px] font-semibold text-paper">Payout Friday</div><div className="mb-2 text-[24px] font-bold tracking-[-.03em] text-peri">{money(Math.round(((seeded ? 4182 : 0) + revenue * 0.92)))}</div><div className="mono text-[10.5px] leading-[1.5] text-paper/50">Held until parcels scan · 8% Kindred fee</div></div>
      ...
      <div className="min-w-0 flex-1 bg-offwhite">
        ...
        <div className="p-4 md:p-[34px] pb-16">
          ...
          {nav === "Messages" && <div className="card rounded-lg p-6 text-[13.5px] text-ink/60">{threads.filter((t) => t.brand === brand.slug).length} conversation{threads.filter((t) => t.brand === brand.slug).length === 1 ? "" : "s"}. <Link href="/messages" className="font-semibold text-navy">Open inbox →</Link></div>}
...
function Overview({ stats, mine, myOrders, seeded, brand }: { stats: { label: string; value: string; delta: string; bg: string; ink: string }[]; mine: Product[]; myOrders: ReturnType<typeof useApp>["orders"]; seeded: boolean; brand: string }) {
  ...
  return (
    <>
      <div className="mb-5 grid grid-cols-2 xl:grid-cols-4 gap-3 md:gap-4">{stats.map((t) => <div key={t.label} className="rounded-2xl p-5 md:p-6" style={{ background: t.bg, color: t.ink, border: t.bg === "#fff" ? "1px solid rgba(18,26,36,.05)" : undefined }}><div className="mb-[14px] text-[10px] font-semibold uppercase tracking-[.14em] opacity-62">{t.label}</div><div className="mb-2 text-[26px] md:text-[34px] font-bold leading-none tracking-[-.04em]">{t.value}</div><div className="mono text-[11.5px] opacity-62">{t.delta}</div></div>)}</div>
      <div className="mb-5 grid gap-5 xl:grid-cols-[1fr_380px] items-start">
        <div className="card rounded-lg p-5 md:p-[26px]"><div className="mb-6 flex items-baseline justify-between"><div className="text-[16px] font-semibold tracking-[-.02em]">Sales</div><div className="mono text-[11.5px] text-ink/42">30 days</div></div><div className="flex h-[170px] items-end gap-[5px]">{DASH.chart.map((h, i) => <div key={i} className="flex-1 rounded-t-[4px]" style={{ height: seeded ? h : Math.max(4, (i > 24 ? myOrders.length * 30 : 0)), background: i > 26 ? "#121A24" : "rgba(28,50,71,.26)" }} />)}</div><div className="mono mt-3 flex justify-between text-[10.5px] text-ink/35"><span>Aug 4</span><span>Aug 18</span><span>Sep 2</span></div></div>
        <div className="card rounded-lg p-6">
          <Label className="mb-4">New post</Label>
          <textarea value={post.caption} onChange={(e) => setPost({ ...post, caption: e.target.value })} placeholder="Say something about this drop…" className="mb-3 min-h-[84px] w-full resize-none rounded-md bg-offwhite p-4 text-[13.5px] leading-[1.6] outline-none placeholder:text-ink/42" />
          ...
          {myPosts.length > 0 && <div className="mt-4 flex flex-col gap-2 border-t border-ink/6 pt-4">{myPosts.slice(0, 3).map((p) => <div key={p.id} className="flex items-center gap-2 text-[12px]"><span className="min-w-0 flex-1 truncate text-ink/65">{p.caption}</span><span className="mono text-ink/40">♡ {p.likes}</span><button onClick={() => deletePost(p.id)} className="text-ink/40">✕</button></div>)}</div>}
        </div>
      </div>
      <div className="grid gap-5 xl:grid-cols-[1fr_380px] items-start">
        <div className="card rounded-lg p-5 md:p-[26px]">
          ...
          <div className="flex flex-col gap-2">{rows.map((r) => <div key={r.slug} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-md bg-offwhite px-4 py-3"><div className="flex min-w-0 items-center gap-3"><Placeholder className="h-11 w-[38px] flex-none rounded-sm" /><span className="truncate text-[13px] font-medium">{r.name}</span></div><span className="text-[13px] font-medium">{money(priceOf(r).price)}</span><span className={clsx("text-[12.5px] font-medium", r.stock !== undefined && r.stock <= 6 ? "text-slate" : "text-ink/60")}>{r.stock === undefined ? "∞" : r.stock === 0 ? "Sold out" : r.stock <= 6 ? `${r.stock} left` : r.stock}</span></div>)}{rows.length === 0 && <div className="text-[13px] text-ink/50">No products yet.</div>}</div>
        </div>
        <div className="card rounded-lg p-5 md:p-6">
          <div className="mb-[18px] flex items-center justify-between"><div className="text-[16px] font-semibold tracking-[-.02em]">Orders</div><span className="rounded-pill bg-slate px-[13px] py-[6px] text-[10.5px] font-semibold uppercase tracking-[.08em] text-paper">{myOrders.filter((o) => o.status === "Placed").length + (seeded ? 6 : 0)} to pack</span></div>
        ...
        <div className="flex flex-col gap-2">
          {mine.map((p) => { const { price, promo } = priceOf(p); return (
            <div key={p.slug} className="grid grid-cols-[1fr_auto] md:grid-cols-[1fr_90px_90px_auto] items-center gap-3 rounded-md bg-offwhite px-4 py-3">
              <div className="flex min-w-0 items-center gap-3"><Placeholder className="h-11 w-[38px] flex-none rounded-sm" /><div className="min-w-0"><div className="truncate text-[13px] font-medium">{p.name}</div><div className="mono text-[10px] text-ink/42">{p.category} · {(p.sizes ?? []).join(" ")}</div></div></div>
              ...
              <span className={clsx("hidden md:inline text-[12.5px] font-medium", p.stock !== undefined && p.stock <= 6 ? "text-slate" : "text-ink/60")}>{p.stock === undefined ? "∞" : p.stock === 0 ? "Sold out" : p.stock}</span>
              <div className="flex gap-1"><button onClick={() => start(p)} className="rounded-pill bg-white px-3 py-[6px] text-[11.5px] font-semibold">Edit</button><button onClick={() => { if (confirm(`Remove ${p.name}?`)) deleteProduct(p.slug); }} className="rounded-pill bg-white px-3 py-[6px] text-[11.5px] font-semibold text-ink/50">✕</button></div>
            </div>); })}
          {mine.length === 0 && <div className="rounded-md bg-offwhite p-8 text-center text-[13px] text-ink/50">Nothing listed yet. Your first product goes live in Explore and search the moment you save it.</div>}
        </div>
      </div>
      {edit && (
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_420px] items-start">
      <div className="card rounded-lg p-4 md:p-[26px]">
        ...
        <div className="flex flex-col gap-2">
          {myPromos.map((p) => <div key={p.id} className="flex flex-wrap items-center gap-3 rounded-md bg-offwhite px-4 py-3"><span className="mono rounded-pill bg-peri px-3 py-1 text-[12px] font-semibold">{p.code}</span><div className="min-w-0 flex-1"><div className="text-[13px] font-medium">{p.pct}% off · {p.label}</div><div className="mono text-[10px] text-ink/42">{p.products === "all" ? "whole catalogue" : `${p.products.length} products`}{p.ends ? ` · ends ${new Date(p.ends).toLocaleDateString(undefined, { month: "short", day: "numeric" })}` : ""}</div></div><button onClick={() => upsertPromo({ ...p, active: !p.active })} className={clsx("rounded-pill px-3 py-[6px] text-[11.5px] font-semibold", p.active ? "bg-ink text-paper" : "bg-white")}>{p.active ? "Active" : "Paused"}</button><button onClick={() => deletePromo(p.id)} className="rounded-pill bg-white px-3 py-[6px] text-[11.5px] font-semibold text-ink/50">✕</button></div>)}
          {myPromos.length === 0 && <div className="rounded-md bg-offwhite p-8 text-center text-[13px] text-ink/50">No promos yet. They show on your brand page, on product cards, and apply at checkout.</div>}
        </div>
      </div>
      <div className="card rounded-lg p-5 md:p-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div><Label className="mb-2">Runs for</Label><div className="flex gap-[6px]">{["3", "7", "14", "30"].map((d) => <button key={d} onClick={() => setF({ ...f, days: d })} className={clsx("rounded-pill px-3 py-[6px] text-[12px] font-medium", f.days === d ? "bg-ink text-paper" : "bg-white soft")}>{d} days</button>)}</div></div>
          <div><Label className="mb-2">Applies to</Label><div className="mb-2 flex gap-[6px]"><button onClick={() => setF({ ...f, all: true })} className={clsx("rounded-pill px-3 py-[6px] text-[12px] font-medium", f.all ? "bg-ink text-paper" : "bg-white soft")}>Whole catalogue</button><button onClick={() => setF({ ...f, all: false })} className={clsx("rounded-pill px-3 py-[6px] text-[12px] font-medium", !f.all ? "bg-ink text-paper" : "bg-white soft")}>Pick products</button></div>{!f.all && <div className="flex flex-wrap gap-[6px]">{mine.map((p) => <button key={p.slug} onClick={() => setF({ ...f, products: f.products.includes(p.slug) ? f.products.filter((x) => x !== p.slug) : [...f.products, p.slug] })} className={clsx("rounded-pill px-3 py-[6px] text-[11.5px]", f.products.includes(p.slug) ? "bg-ink text-paper" : "bg-white soft")}>{p.name}</button>)}</div>}</div>
...
function Drops({ brand, mine, myDrops }: { brand: string; mine: Product[]; myDrops: Drop[] }) {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_420px] items-start">
      <div className="flex flex-col gap-3">
        {myDrops.map((d) => { const past = new Date(d.at).getTime() < now; return <div key={d.id} className={clsx("rounded-lg p-6", past ? "card" : "bg-ink text-paper")}><div className="mb-1 flex items-center justify-between gap-3"><div className="text-[20px] font-bold tracking-[-.03em]">{d.title}</div><button onClick={() => deleteDrop(d.id)} className={clsx("text-[12px] font-semibold", past ? "text-ink/45" : "text-paper/60")}>Remove</button></div><div className={clsx("mb-4 text-[13px]", past ? "text-ink/55" : "text-paper/70")}>{new Date(d.at).toLocaleString(undefined, { weekday: "long", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })} · {d.pieces} pieces · {notify.includes(d.id) ? 1 : 0}{past ? "" : " + "}{past ? "" : Math.round(d.pieces * 2.3)} waiting</div>{!past && <Countdown at={d.at} dark />}</div>; })}
        {myDrops.length === 0 && <div className="card rounded-lg p-8 text-center text-[13px] text-ink/50">No drops scheduled. A drop pins to the top of the Drops feed for everyone who follows you.</div>}
      </div>
      <div className="card rounded-lg p-5 md:p-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div><Label className="mb-2">Products in the drop</Label><div className="flex flex-wrap gap-[6px]">{mine.map((p) => <button key={p.slug} onClick={() => setF({ ...f, products: f.products.includes(p.slug) ? f.products.filter((x) => x !== p.slug) : [...f.products, p.slug] })} className={clsx("rounded-pill px-3 py-[6px] text-[11.5px]", f.products.includes(p.slug) ? "bg-ink text-paper" : "bg-white soft")}>{p.name}</button>)}{mine.length === 0 && <span className="text-[12px] text-ink/45">Add products first.</span>}</div></div>
      ...
      <div className="flex flex-col gap-2">
        {myOrders.map((o) => { const items = o.items.filter((i) => i.brand === brand); return (
          <div key={o.id} className="flex flex-wrap items-center gap-3 rounded-md bg-offwhite px-4 py-3">
            ...
            <span className={clsx("rounded-pill px-3 py-[6px] text-[10.5px] font-semibold uppercase tracking-[.08em]", o.status === "Delivered" ? "bg-white text-ink/55" : "bg-sky")}>{o.status}</span>
        ...
        {seeded && DASH.orderRows.map((o) => <div key={o.meta} className="flex items-center gap-3 rounded-md bg-offwhite px-4 py-3"><Avatar init={o.init} tint={o.tint} ink={o.ink} size={36} /><div className="min-w-0 flex-1"><div className="text-[13.5px] font-medium">{o.name}</div><div className="mono text-[10.5px] text-ink/42">{o.meta}</div></div><span className="text-[13px] font-medium">{o.total}</span><span className="rounded-pill bg-sky px-3 py-[6px] text-[10.5px] font-semibold uppercase tracking-[.08em]">To pack</span></div>)}
        {!seeded && myOrders.length === 0 && <div className="rounded-md bg-offwhite p-8 text-center text-[13px] text-ink/50">No orders yet.</div>}
      </div>
    </div>
  );
}
...
function LookbookBuilder({ brand, mine }: { brand: string; mine: Product[] }) {
  ...
  const [f, setF] = useState({ title: "", season: "", blurb: "", bg: "#DCD5C7", frames: [{ ...EMPTY_FRAME }, { ...EMPTY_FRAME, h: 320 }, { ...EMPTY_FRAME, h: 480 }] as LookFrame[] });
  const setFrame = (i: number, patch: Partial<LookFrame>) => setF((p) => ({ ...p, frames: p.frames.map((fr, j) => (j === i ? { ...fr, ...patch } : fr)) }));
  const save = () => { if (!f.title.trim()) return; const slug = slugify(`${f.title}-${brand.split("-")[0]}`); upsertLookbook({ slug, brand, title: f.title.trim(), season: f.season.trim() || "No season", blurb: f.blurb.trim(), bg: f.bg, frames: f.frames.filter((fr) => fr.image || fr.product || fr.label), createdAt: new Date().toISOString() }); setF({ title: "", season: "", blurb: "", bg: "#DCD5C7", frames: [{ ...EMPTY_FRAME }, { ...EMPTY_FRAME, h: 320 }, { ...EMPTY_FRAME, h: 480 }] }); };
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_460px] items-start">
      <div className="flex flex-col gap-3">
        {books.map((l) => { const c = lookCount(l); const dark = l.bg === "#121A24"; return <div key={l.slug} className="flex items-center justify-between gap-4 rounded-lg p-5" style={{ background: l.bg, color: dark ? "#F6F4EF" : "#121A24" }}><div><div className="mb-1 text-[10px] font-semibold uppercase tracking-[.14em] opacity-60">{l.season}</div><div className="text-[20px] font-bold tracking-[-.03em]">{l.title}</div><div className="mono text-[11px] opacity-60">{c.looks} looks · {c.shoppable} shoppable</div></div><div className="flex gap-2"><Link href={`/lookbook/${l.slug}`} className="rounded-pill bg-white/80 px-3 py-[6px] text-[11.5px] font-semibold text-ink">View</Link>{l.createdAt && <button onClick={() => deleteLookbook(l.slug)} className="rounded-pill bg-white/60 px-3 py-[6px] text-[11.5px] font-semibold text-ink/60">✕</button>}</div></div>; })}
        {books.length === 0 && <div className="card rounded-lg p-8 text-center text-[13px] text-ink/50">No lookbooks yet. Build one on the right: each frame can carry an image URL and one shoppable hotspot.</div>}
      </div>
      <div className="card rounded-lg p-5 md:p-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div><Label className="mb-2">Cover colour</Label><div className="flex gap-2">{["#DCD5C7", "#E5DFD3", "#121A24", "#7C8C6F"].map((c) => <button key={c} onClick={() => setF({ ...f, bg: c })} className="h-8 w-8 rounded-pill" style={{ background: c, boxShadow: f.bg === c ? "0 0 0 2px #F6F4EF,0 0 0 3.5px #121A24" : undefined }} />)}</div></div>
          <Label>Frames</Label>
          {f.frames.map((fr, i) => (
            <div key={i} className="rounded-md bg-offwhite p-3">
              <div className="mb-2 flex items-center justify-between"><span className="text-[12px] font-semibold">Frame {i + 1}</span><button onClick={() => setF({ ...f, frames: f.frames.filter((_, j) => j !== i) })} className="text-[11px] text-ink/45">Remove</button></div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <label className="flex items-center gap-1 text-[11px] text-ink/55">x <input type="range" min={5} max={95} value={fr.x ?? 50} onChange={(e) => setFrame(i, { x: Number(e.target.value) })} className="w-16 accent-black" /></label>
                <label className="flex items-center gap-1 text-[11px] text-ink/55">y <input type="range" min={5} max={95} value={fr.y ?? 45} onChange={(e) => setFrame(i, { y: Number(e.target.value) })} className="w-16 accent-black" /></label>
                <label className="flex items-center gap-1 text-[11px] text-ink/55">h <input type="range" min={240} max={600} step={20} value={fr.h} onChange={(e) => setFrame(i, { h: Number(e.target.value) })} className="w-16 accent-black" /></label>
              </div>
            </div>
          ))}
          <button onClick={() => setF({ ...f, frames: [...f.frames, { ...EMPTY_FRAME }] })} className="rounded-md border-[1.5px] border-dashed border-ink/16 py-3 text-[12.5px] font-medium text-ink/55">+ Add frame</button>
