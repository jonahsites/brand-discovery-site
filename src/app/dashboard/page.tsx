    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_420px] items-start">
      ...
      <div className="card rounded-lg p-5 md:p-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div><Label className="mb-2">Runs for</Label><div className="flex gap-[6px]">{["3", "7", "14", "30"].map((d) => <button key={d} onClick={() => setF({ ...f, days: d })} className={clsx("rounded-pill border px-3 py-[6px] text-[12px] font-medium", f.days === d ? "bg-ink text-white border-black" : "bg-white border-ink/10")}>{d} days</button>)}</div></div>
          <div><Label className="mb-2">Applies to</Label><div className="mb-2 flex gap-[6px]"><button onClick={() => setF({ ...f, all: true })} className={clsx("rounded-pill border px-3 py-[6px] text-[12px] font-medium", f.all ? "bg-ink text-white border-black" : "bg-white border-ink/10")}>Whole catalogue</button><button onClick={() => setF({ ...f, all: false })} className={clsx("rounded-pill border px-3 py-[6px] text-[12px] font-medium", !f.all ? "bg-ink text-white border-black" : "bg-white border-ink/10")}>Pick products</button></div>{!f.all && <div className="flex flex-wrap gap-[6px]">{mine.map((p) => <button key={p.slug} onClick={() => setF({ ...f, products: f.products.includes(p.slug) ? f.products.filter((x) => x !== p.slug) : [...f.products, p.slug] })} className={clsx("rounded-pill border px-3 py-[6px] text-[11.5px]", f.products.includes(p.slug) ? "bg-ink text-paper border-navy" : "bg-white border-ink/10")}>{p.name}</button>)}</div>}</div>
