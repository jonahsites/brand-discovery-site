  ...
  return (
    <div className="flex min-h-screen">
      <aside className="hidden md:flex w-[252px] flex-none flex-col bg-ink p-5 pt-7 sticky top-0 h-screen">
        ...
        <div className="mt-auto rounded-md bg-paper/10 p-[18px]"><div className="mb-[6px] text-[12.5px] font-semibold text-paper">Payouts</div><div className="mb-2 text-[24px] font-extrabold tracking-[-.03em] text-moss">{money(Math.round(revenue))}</div><div className="mono text-[10.5px] leading-[1.5] text-paper/50">Payouts start once built-in payment is live.</div></div>
      ...
      <div className="min-w-0 flex-1 bg-paper">
        <div className="md:hidden sticky top-0 z-30 bg-ink px-[18px] pb-3 pt-4">
          <div className="flex items-center gap-[10px]"><div className="grid h-[30px] w-[30px] place-items-center rounded-[8px] text-[10.5px] font-extrabold" style={{ background: brand.tint, color: brand.ink }}>{brand.init}</div><div className="flex-1 text-[13.5px] font-semibold text-paper">{brand.name}</div><Link href="/" className="grid h-[34px] w-[34px] place-items-center rounded-pill bg-paper/14 text-[14px] text-paper">✕</Link></div>
          <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto">{NAV.map((l) => <button key={l} onClick={() => setNav(l)} className={clsx("flex-none rounded-pill px-3 py-[7px] text-[12px] font-semibold", nav === l ? "bg-paper text-ink" : "bg-paper/12 text-paper/70")}>{l}</button>)}</div>
        </div>
    ...
    <>
      ...
      <div className="mb-5 grid gap-5 xl:grid-cols-[1fr_380px] items-start">
        ...
        <div className="card rounded-lg p-6">
          ...
          <textarea value={post.caption} onChange={(e) => setPost({ ...post, caption: e.target.value })} placeholder="Say something about this drop…" className="mb-3 min-h-[84px] w-full resize-none rounded-md bg-cream p-4 text-[13.5px] leading-[1.6] outline-none placeholder:text-ink/42" />
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_380px] items-start">
        <div className="card rounded-lg p-5 md:p-[26px]">
          ...
          <div className="flex flex-col gap-2">{rows.map((r) => <div key={r.slug} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-md bg-cream px-4 py-3"><div className="flex min-w-0 items-center gap-3"><Placeholder className="h-11 w-[38px] flex-none rounded-sm" /><span className="truncate text-[13px] font-medium">{r.name}</span></div><span className="text-[13px] font-medium">{money(priceOf(r).price)}</span><span className={clsx("text-[12.5px] font-medium", r.stock !== undefined && r.stock <= 6 ? "text-sage" : "text-ink/60")}>{r.stock === undefined ? "∞" : r.stock === 0 ? "Sold out" : r.stock <= 6 ? `${r.stock} left` : r.stock}</span></div>)}{rows.length === 0 && <div className="text-[13px] text-ink/50">No products yet.</div>}</div>
        </div>
      ...
      <div className="card rounded-lg p-4 md:p-[26px]">
        ...
        <div className="flex flex-col gap-2">
          {mine.map((p) => { const { price, promo } = priceOf(p); return (
            <div key={p.slug} className="grid grid-cols-[1fr_auto] md:grid-cols-[1fr_90px_90px_auto] items-center gap-3 rounded-md bg-cream px-4 py-3">
              ...
              <span className="hidden md:inline text-[13px] font-medium">{money(price)}{promo && <span className="ml-1 text-[10px] text-ink">−{promo.pct}%</span>}</span>
              <span className={clsx("hidden md:inline text-[12.5px] font-medium", p.stock !== undefined && p.stock <= 6 ? "text-sage" : "text-ink/60")}>{p.stock === undefined ? "∞" : p.stock === 0 ? "Sold out" : p.stock}</span>
          ...
          {mine.length === 0 && <div className="rounded-md bg-cream p-8 text-center text-[13px] text-ink/50">Nothing listed yet. Your first product goes live in Explore and search the moment you save it.</div>}
        </div>
      </div>
      {edit && (
        <div className="card rounded-lg p-5 md:p-6 xl:sticky xl:top-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label className="mb-2">Pre-order · ships on</Label><input type="date" className={inputCls} value={edit.preorder ? edit.preorder.slice(0, 10) : ""} onChange={(e) => setEdit({ ...edit, preorder: e.target.value ? new Date(e.target.value + "T12:00:00").toISOString() : undefined })} /></div>
            </div>
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_420px] items-start">
      ...
      <div className="card rounded-lg p-5 md:p-6">
        ...
        <div className="flex flex-col gap-3">
          <div><Label className="mb-2">Title</Label><input className={inputCls} value={f.title} onChange={(e) => setF({ ...f, title: e.target.value })} placeholder="Bone colorway" /></div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="mt-4 rounded-md bg-cream p-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div><Label className="mb-2">Accent color · hex</Label><div className="flex items-center gap-3"><input className={clsx(inputCls, "!w-[130px]")} value={f.accent} onChange={(e) => setF({ ...f, accent: e.target.value })} placeholder="#3A5A3F" /><span className="h-9 w-9 flex-none rounded-md" style={{ background: f.accent || "var(--sage)" }} /></div></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_460px] items-start">
      ...
      <div className="card rounded-lg p-5 md:p-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div><Label className="mb-2">Cover color</Label><div className="flex gap-2">{["#DCD5C7", "#E5DFD3", "#121A24", "#7C8C6F"].map((c) => <button key={c} onClick={() => setF({ ...f, bg: c })} className="h-8 w-8 rounded-pill" style={{ background: c, boxShadow: f.bg === c ? "0 0 0 2px #F6F4EF,0 0 0 3.5px #121A24" : undefined }} />)}</div></div>
          <Label>Frames</Label>
          {f.frames.map((fr, i) => (
            <div key={i} className="rounded-md bg-cream p-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <label className="flex items-center gap-1 text-[11px] text-ink/55">x <input type="range" min={5} max={95} value={fr.x ?? 50} onChange={(e) => setFrame(i, { x: Number(e.target.value) })} className="w-16 accent-ink" /></label>
                <label className="flex items-center gap-1 text-[11px] text-ink/55">y <input type="range" min={5} max={95} value={fr.y ?? 45} onChange={(e) => setFrame(i, { y: Number(e.target.value) })} className="w-16 accent-ink" /></label>
                <label className="flex items-center gap-1 text-[11px] text-ink/55">h <input type="range" min={240} max={600} step={20} value={fr.h} onChange={(e) => setFrame(i, { h: Number(e.target.value) })} className="w-16 accent-ink" /></label>
              </div>
            </div>
          ))}
          <button onClick={() => setF({ ...f, frames: [...f.frames, { ...EMPTY_FRAME }] })} className="rounded-md bg-cream py-3 text-[12.5px] font-semibold text-ink/60">+ Add frame</button>
