    ...
    <>
      ...
      <Page className="pt-6">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div><h1 className="mb-[5px] text-[26px] md:text-[30px] font-bold leading-[1.05] tracking-[-.035em]">{style === "All" ? "Every brand on Kindred" : `${style} brands`}</h1><div className="text-[12.5px] text-ink/48">{list.length} independent label{list.length === 1 ? "" : "s"} · {list.filter((b) => brandTier(b.followers) === "Indie").length} under 1k followers</div></div>
          <div className="flex gap-2"><button onClick={() => setShowFilters(!showFilters)} className={clsx("lg:hidden rounded-pill border px-4 py-[10px] text-[12.5px] font-semibold", showFilters ? "bg-ink text-white border-black" : "bg-white border-ink/10")}>≡ Filters{tier.length + values.length + (country !== "All" ? 1 : 0) > 0 ? ` · ${tier.length + values.length + (country !== "All" ? 1 : 0)}` : ""}</button><select value={sort} onChange={(e) => setSort(e.target.value)} className="rounded-pill border border-ink/10 bg-white px-[18px] py-[10px] text-[12.5px] font-medium outline-none">{SORTS.map((s) => <option key={s}>{s}</option>)}</select></div>
        </div>
        <div className="grid gap-6 lg:grid-cols-[240px_1fr] items-start">
          <aside className={clsx("flex-col gap-6 lg:sticky lg:top-[150px] lg:flex", showFilters ? "flex" : "hidden")}>
            <div><Label className="mb-3">Brand size</Label><div className="flex flex-wrap gap-[6px]">{TIERS.map((t) => <button key={t} onClick={() => tog(tier, t, setTier)} className={clsx("rounded-pill border px-3 py-[7px] text-[12px] font-medium", tier.includes(t) ? "bg-ink text-white border-black" : "bg-white border-ink/10")}>{t}</button>)}</div></div>
            <div><Label className="mb-3">Based in</Label><div className="flex flex-wrap gap-[6px]">{countries.map((c) => <button key={c} onClick={() => setCountry(c)} className={clsx("rounded-pill border px-3 py-[7px] text-[12px] font-medium", country === c ? "bg-ink text-white border-black" : "bg-white border-ink/10")}>{c}</button>)}</div></div>
            <div><Label className="mb-3">Values</Label><div className="flex flex-wrap gap-[6px]">{VALUE_OPTIONS.map((v) => <button key={v} onClick={() => tog(values, v, setValues)} className={clsx("rounded-pill border px-3 py-[7px] text-[12px] font-medium", values.includes(v) ? "bg-ink text-paper border-navy" : "bg-white border-ink/10")}>{v}</button>)}</div></div>
            <Link href="/sell" className="rounded-xl bg-peri p-5"><div className="mb-1 text-[14px] font-semibold tracking-[-.02em]">Your brand isn&apos;t here?</div><div className="text-[12px] leading-[1.5] text-ink/60">Apply in five minutes. Your onboarding answers become these filters.</div></Link>
          </aside>
            ...
            {list.map((b) => { const own = products.filter((p) => p.brand === b.slug); const promo = promos.find((p) => p.active && p.brand === b.slug); const drop = drops.find((d) => d.brand === b.slug); return (
              <div key={b.slug} className="card overflow-hidden rounded-lg lift">
                ...
                <div className="p-4 pt-0">
                  ...
                  <div className="flex gap-2"><FollowButton slug={b.slug} size="sm" className="flex-1" /><Link href={`/brand/${b.slug}`} className="grid h-[34px] w-[34px] place-items-center rounded-pill border border-ink/10 bg-white text-[13px]">↗</Link></div>
                </div>
              </div>); })}
