    ...
    <Page className="pt-0 md:pt-6">
      ...
      {(drop || promo) && (
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {drop && <div className="rounded-lg bg-navy p-6 text-paper"><Label light className="mb-2">Next drop</Label><div className="mb-1 text-[22px] font-bold tracking-[-.03em]">{drop.title}</div><div className="mb-4 text-[13px] text-paper/70">{drop.pieces} pieces · {drop.blurb}</div><Countdown at={drop.at} dark /></div>}
          {promo && <div className="rounded-lg bg-peri p-6"><Label className="mb-2">Promo running</Label><div className="mb-1 text-[22px] font-bold tracking-[-.03em]">{promo.pct}% off · {promo.label}</div><div className="text-[13px] text-ink/60" suppressHydrationWarning>Use code <span className="mono font-semibold text-ink">{promo.code}</span> at checkout{promo.ends ? ` · ends ${new Date(promo.ends).toLocaleDateString(undefined, { month: "short", day: "numeric" })}` : ""}</div></div>}
        </div>
      )}
      ...
      {tab === "Shop" && (own.length ? <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-5 xl:grid-cols-4">{own.map((p) => <ProductCard key={p.slug} p={p} showBrand={false} tall />)}</div>
        : <div className="card rounded-lg p-10 text-center text-[14px] text-ink/55">{isOwner ? <>No products yet. <Link href="/dashboard?tab=Products" className="font-semibold text-navy">Add your first piece →</Link></> : "This brand hasn't listed anything yet. Follow to hear about the first drop."}</div>)}
      {tab === "Posts" && (
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr] items-start">
          <div className="card rounded-lg p-6 md:p-[38px]">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {b.values.map((t, i) => <span key={t} className={`rounded-pill px-[18px] py-[10px] text-[12px] font-semibold ${i === 0 ? "bg-peri" : i === 1 ? "bg-sky" : "bg-offwhite border border-ink/7"}`}>{t}</span>)}
            </div>
          </div>
