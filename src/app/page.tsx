...
function HomeInner() {
  ...
  const hero = {
    Dashboard: { kicker: promo ? `${promo.label} · ${promoBrand?.name}` : "This week on Kindred", t1: promo ? "Get up to" : "Find your next", t2: promo ? `${promo.pct}% off` : "favourite brand", cta: promo ? "Get discount" : "Start exploring", foot: promo?.ends ? `Valid until ${new Date(promo.ends).toLocaleDateString(undefined, { day: "2-digit", month: "long", year: "numeric" })}` : `${brands.length} independent brands live`, href: promo ? `/brand/${promo.brand}` : "/explore", img: products.find((p) => p.brand === featBrand.slug && p.image)?.image },
