      ...
      <div className="grid gap-7 lg:grid-cols-[1fr_400px] items-start">
        <div>
          <div className={clsx(tab !== "Saved" && "hidden md:block")}>
            ...
            <div className="mb-9 grid grid-cols-2 gap-3 md:gap-4 lg:grid-cols-3">{savedP.map((p) => <ProductCard key={p.slug} p={p} />)}{savedP.length === 0 && <div className="col-span-full card rounded-lg p-10 text-center"><div className="mb-3 mx-auto grid h-14 w-14 place-items-center rounded-pill" style={{background:"var(--cream)"}}><span className="text-[20px]">♡</span></div><h3 className="mb-1 text-[22px] tracking-[-.015em]" style={{fontFamily:"var(--font-instrument), Georgia, serif"}}>Nothing saved yet</h3><p className="mb-4 text-[13px] text-ink/55">Tap ♡ on anything you like the look of. It lives here until you buy it.</p><Link href="/explore" className="press inline-flex rounded-sm bg-ink px-5 py-[10px] text-[11.5px] font-semibold text-paper">Start browsing</Link></div>}</div>
          </div>
