...
function AccountInner() {
  ...
  return (
    <Page className="pt-6 md:pt-9">
      ...
      <div className="mb-[22px] flex gap-2 md:hidden">{["Saved", "Orders", "Profile"].map((t) => <button key={t} onClick={() => setTab(t)} className={clsx("flex-1 rounded-pill py-3 text-center text-[12.5px] font-semibold", tab === t ? "bg-black text-white" : "bg-white border border-black/10")}>{t}</button>)}</div>
      <div className="grid gap-7 lg:grid-cols-[1fr_400px] items-start">
          ...
          <div className={clsx(tab !== "Orders" && "hidden md:block")}>
            ...
            {(alerts.length > 0 || notify.length > 0) && (
              <div className="mb-9"><h3 className="mb-4 text-[20px] font-semibold tracking-[-.025em]">Watching</h3><div className="flex flex-col gap-2">
                {alerts.map((s) => { const p = products.find((x) => x.slug === s); return p ? <Link key={s} href={`/product/${s}`} className="card flex items-center justify-between rounded-md px-4 py-3 text-[13px]"><span>◔ Price alert · {p.name}</span><span className="text-black/45">{money(p.price)}</span></Link> : null; })}
                {notify.map((id) => { const d = drops.find((x) => x.id === id); return d ? <div key={id} className="card flex items-center justify-between rounded-md px-4 py-3 text-[13px]"><span>◔ Drop reminder · {d.title}</span><span className="text-black/45">{new Date(d.at).toLocaleDateString(undefined, { weekday: "short", day: "numeric", month: "short" })}</span></div> : null; })}
              </div></div>
            )}
          </div>
        </div>
        <div className={clsx("flex flex-col gap-4", tab !== "Profile" && "hidden md:flex")}>
          ...
          <div className="rounded-lg bg-navy p-6 md:p-7 text-offwhite">
            ...
            {addTag && <div className="mt-3 flex flex-wrap gap-2 border-t border-offwhite/15 pt-3">{STYLE_OPTIONS.filter((s) => !styleTags.includes(s)).map((s) => <button key={s} onClick={() => setStyleTags([...styleTags, s])} className="rounded-pill bg-offwhite/8 px-3 py-[7px] text-[11.5px] text-offwhite/80">+ {s}</button>)}</div>}
          </div>
          <div className="card rounded-lg p-6 md:p-7">
            ...
            <div className="flex flex-col gap-[14px]">
              {([["Tops", "tops", SIZE_LADDER.slice(1, 7)], ["Trousers · waist", "waist", ["28", "30", "32", "33", "34", "36"]], ["Shoes · EU", "shoe", ["40", "41", "42", "43", "44", "45"]]] as const).map(([label, key, opts]) => (
                <div key={key}><div className="mb-2 text-[12px] text-black/50">{label}</div><div className="flex flex-wrap gap-[7px]">{opts.map((v) => <button key={v} onClick={() => setSizes({ ...sizes, [key]: v })} className={clsx("rounded-pill px-[13px] py-[8px] text-[12.5px] font-medium", sizes[key] === v ? "bg-sky" : "bg-offwhite")}>{v}</button>)}</div></div>
              ))}
            </div>
            <div className="mt-[18px] rounded-[10px] bg-offwhite px-[18px] py-[14px] text-[12px] leading-[1.5] text-black/55">We hide anything sold out in your sizes.</div>
          </div>
          <div className="card rounded-lg p-6 md:p-7">
            <Label className="mb-[18px]">Following · {following.length}</Label>
            <div className="flex flex-col gap-3">
              {following.map((b) => <div key={b.slug} className="flex items-center gap-3"><Avatar init={b.init} tint={b.tint} ink={b.ink} size={40} /><Link href={`/brand/${b.slug}`} className="min-w-0 flex-1"><div className="text-[13px] font-semibold">{b.name}</div><div className="mono text-[10.5px] text-black/42">{brandMeta(b)}</div></Link><button onClick={() => toggleFollow(b.slug)} className="rounded-pill bg-offwhite px-[14px] py-[7px] text-[11.5px] font-semibold">Following</button></div>)}
