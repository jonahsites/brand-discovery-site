...
export default function ProductCard({ p, showBrand = true, compact, hoverAdd, tall }: { p: Product; showBrand?: boolean; compact?: boolean; hoverAdd?: boolean; tall?: boolean }) {
  ...
  return (
    <div className={clsx("card group relative lift", compact ? "rounded-xl p-2" : "rounded-2xl p-2 md:p-[10px]")}>
      ...
      <button aria-label={saved ? "Unsave" : "Save"} onClick={() => { toggleSaved(p.slug); toast(saved ? "Removed from saved" : "Saved", saved ? undefined : "/account"); }} className={clsx("press absolute grid place-items-center rounded-pill text-[14px]", compact ? "right-4 top-4 h-[30px] w-[30px]" : "right-[18px] top-[18px] h-8 w-8", saved ? "bg-black text-white" : "glass-chip")}>{saved ? "♥" : "♡"}</button>
      {hoverAdd && !soldOut && (
      ...
      <div className={clsx(compact ? "px-1 pt-[9px] pb-[3px]" : "px-[6px] pt-[10px] pb-1")}>
