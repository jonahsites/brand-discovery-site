  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="glass absolute bottom-3 right-3 top-3 left-3 md:left-auto md:w-[436px] overflow-auto rounded-lg p-5 md:p-[26px]">
        ...
        {bagGroups.length === 0 && <div className="rounded-xl bg-white/70 p-6 text-center text-[13.5px] text-ink/55">Your bag is empty. <Link href="/explore" className="font-semibold text-navy" onClick={() => openBag(false)}>Explore brands →</Link></div>}
        {bagGroups.map((g) => (
          <div key={g.brand.slug} className="mb-3 rounded-xl border border-white/80 bg-white/68 p-4">
            ...
            {g.items.map((it) => (
              <div key={it.key} className="flex items-center gap-3 py-2">
                <Placeholder className="h-16 w-14 flex-none rounded-[8px]" />
