...
- Design language (Kindred, from the Claude Design file in `docs/design/Kindred.dc.html`): rounded (12/20/32px + pills), off-white `#F6F7F9`, sky `#C7DCEF`, periwinkle `#DBE1EF`, slate `#456F94`, navy `#1C3247`, near-black `#121212`. Schibsted Grotesk + JetBrains Mono. Glass only on floating chrome. Tokens and `glass` utilities in `src/app/globals.css`.
